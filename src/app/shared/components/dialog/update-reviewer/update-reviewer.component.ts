...
@Component({
  ...
  styleUrls: ['./update-reviewer.component.scss']
})
export class UpdateReviewerComponent implements OnInit {
  ...
  constructor(public dialogref: MatDialogRef<UpdateReviewerComponent>, @Inject(MAT_DIALOG_DATA) public data: any,
  ...
  getSelected(event) {
    this.updateCreateDialogueUserForm.get('listIdBrancheCode').setValue(null);
    this.updateCreateDialogueUserForm.get('listIdBrancheCode').setValidators(Validators.required);
  ...
  selectchange(ob){
    this.countryVar = ob.value;
     ...
     this.cities=Object.values(countries[this.countryVar]);
     this.updateCreateDialogueUserForm.get('region').setValue(null);
     this.updateCreateDialogueUserForm.get('region').setValidators(Validators.required);
     this.filteredOption = this.updateCreateDialogueUserForm.get('country').valueChanges.pipe(startWith(''), map(value => this._filterCountry(value)));
     this.filteredCitiesOption = this.updateCreateDialogueUserForm.get('region').valueChanges.pipe(startWith(''), map(value => this._filterCity(value)));
